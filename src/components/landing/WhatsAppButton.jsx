...
const content = {
  ro: {
    tooltip: 'Hai să vorbim pe WhatsApp',
    message: 'Salut Iulian! Sunt interesat de coaching-ul pentru executivi.'
  },
  en: {
    tooltip: 'Let\'s chat on WhatsApp',
    message: 'Hi Iulian! I\'m interested in executive coaching.'
  },
  fr: {
    tooltip: 'Discutons sur WhatsApp',
    message: 'Bonjour Iulian! Je suis intéressé par le coaching pour dirigeants.'
  }
};

export default function WhatsAppButton({ lang = 'ro' }) {
  const [showTooltip, setShowTooltip] = useState(false);
  const [hasInteracted, setHasInteracted] = useState(false);
  const t = content[lang] || content.en;
  
  const phoneNumber = '40750497638';
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50">
        ...
        {showTooltip && (
          ...
          >
            <div className="relative bg-white rounded-2xl shadow-2xl shadow-black/10 p-4 border border-slate-100">
              ...
              <p className="text-sm text-slate-700 font-medium pr-4">{t.tooltip}</p>
              {/* Arrow */}
      ...
      <motion.a
        ...
        className="group flex items-center justify-center w-14 h-14 bg-[#25D366] rounded-full shadow-lg shadow-[#25D366]/30 hover:shadow-xl hover:shadow-[#25D366]/40 hover:scale-110 transition-all duration-300"
